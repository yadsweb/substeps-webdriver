...
public class CustomWebDriverFactory implements WebDriverFactory {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(CustomWebDriverFactory.class);
    ...
    public WebDriver createWebDriver() {
        ...
        switch (WebdriverSubstepsConfiguration.driverType()) {
            ...
            case REMOTE: {
                URL HubUrl = null;
                try {
                    HubUrl = new URL(Configuration.INSTANCE.getString("hub.url"));
                } catch (MalformedURLException ex) {
                    java.util.logging.Logger.getLogger(CustomWebDriverFactory.class.getName()).log(Level.SEVERE, "Error when trying to creat the hub url! Check the \".properties\" file", ex);
                }

                Platform platform = Platform.valueOf(Configuration.INSTANCE.getString("capabilities.platform"));
                DesiredCapabilities capabilities_remote = new DesiredCapabilities(Configuration.INSTANCE.getString("capabilities.browse"), Configuration.INSTANCE.getString("capabilities.version"), platform);
                webDriver = new RemoteWebDriver(HubUrl, capabilities_remote);
    ...
    /**
     * By default the HtmlUnit driver is set to en-us. This can cause problems with formatters.
     */
